...
# Works on macOS (Apple Silicon or Intel). 8 GB RAM minimum.
#
set -e
...
if [[ "$OSTYPE" != "darwin"* ]]; then
  die "This installer currently supports macOS only. Windows/Linux support is coming."
fi
ok "macOS detected"

# -------- 2. Homebrew --------
if ! command -v brew >/dev/null 2>&1; then
  say "Installing Homebrew (one-time, ~2 min). You may be asked for your Mac password."
